...
u8 u3receive_short_ok = 0;			//串口3接收8字节完成标志
u8 u3receive_long_ok = 0;			//串口3接收>8字节完成标志
u16 u3receive_len = 0;				//串口3     8字节接收计数器
u8 receive_short_ok = 0;			//串口2接收8字节完成标志
u8 receive_long_ok = 0;				//串口2接收>8字节完成标志
u16 receive_len = 0;				//串口2     8字节接收计数器
//u8 i = 0;					//for循环用变量
Serialize_Pack receive_pack;
Show_Text show_text;
u8 receiveMode = 0;				//接收模式 0: 8字节  1: >8字节
u16 dataLen = 0;						//>8字节接收计数器
u8 receiveMore[200];				//>8字节接收缓存


//=========全局变量==========
u8 gTxBuf[207];				 	//串行发送缓存
...
void ResetFingerFlag(void)		//指纹模块标志位复位
{
	...
	receive_len = 0;			//8字节接收计数器u8 receiveMode = 0;			//接收模式 0:8字节  1:>8字节
